...
public abstract class Scheduler {
	...
	void traitement(User userChoisi, int i, int j){
		double debitUser = userChoisi.getTabDebits()[i][j];
		int energie;
		...
		pa.sumURUtilise();
		if(userChoisi.getDistance()){
			pa.sumURUtiliseProche();
			pa.sumEnergieProche(energie);
			pa.sumDebitProche(debitUser);
		}else{
			pa.sumDebitLoin(debitUser);
			pa.sumURUtiliseLoin();
			pa.sumEnergieLoin(energie);
		}
		//Gestion des paquets
